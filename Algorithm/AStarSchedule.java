    ...
    public AStarSchedule(List<Dust> dustList)
    {
        ...
        for(int i = 0; i < size; i++)
        {
            for(int j = 0; j < i; j++)
            {
                ...
                {
                    //Khoảng cách từ i -> j
                    distanceMap[i][j] = distance(dustList.get(i).getPxy(), this.dustList.get(j).getPxy());
...
//        }
    ...
    private int[] createSchedule(List<Dust> listDust)
    {
        ...
        // 0 <=> listDust.get(0) ...
        int[] initialPath = {};
        Node start = new Node(initialPath, 0, 0, listDust.get(0), 0);
            ...
            if(path != null)
            {
                ...
                for (int i = 0; i < size; i++) 
                {
                    if(neighbours[i] != 0) //Các node hàng xóm không thuộc path (chưa duyệt)
                    {
                        ...
                        Node node = new Node(newPath, gCost, fCost, listDust.get(i), i);
                        openList.add(node);
                    }
                }
